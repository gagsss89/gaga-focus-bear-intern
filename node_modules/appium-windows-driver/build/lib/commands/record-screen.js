...
exports.windowsStartRecordingScreen = windowsStartRecordingScreen;
exports.windowsStopRecordingScreen = windowsStopRecordingScreen;
...
class ScreenRecorder {
    ...
    async start() {
        ...
        this.log.debug(`Starting ${FFMPEG_BINARY}: ${support_1.util.quote(fullCmd)}`);
        ...
        try {
            ...
            await this._enforceTermination();
            throw this.log.errorWithException(`The expected screen record file '${this._videoPath}' does not exist. ` +
 ...
 */
async function windowsStartRecordingScreen(timeLimit, videoFilter, fps, preset, captureCursor, captureClicks, audioInput, forceRestart = true) {
 ...
 */
async function windowsStopRecordingScreen(remotePath, user, pass, method, headers, fileFieldName, formFields) {
...
async function startRecordingScreen(options = {}) {
    ...
    await this.windowsStartRecordingScreen(timeLimit, videoFilter, fps, preset, captureCursor, captureClicks, audioInput, forceRestart);
}
/**
 * Stop recording the screen.
 * If no screen recording has been started before then the method returns an empty string.

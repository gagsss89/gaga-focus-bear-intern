...
const PORT_ALLOCATION_GUARD = support_1.util.getLockFileGuard(path_1.default.resolve(os_1.default.tmpdir(), 'wad_port_guard'), {
    timeout: 5,
    tryRecovery: true,
});
const TROUBLESHOOTING_LINK = 'https://github.com/appium/appium-windows-driver?tab=readme-ov-file#troubleshooting';
class WADProxy extends driver_1.JWProxy {
    ...
    async isListening() {
        ...
        try {
            await (0, portscanner_1.checkPortStatus)(parseInt(parsedUrl.port, 10) || defaultPort, parsedUrl.hostname);
            return true;
        }
        catch {
            return false;
    ...
    async proxyCommand(url, method, body = null) {
        if (this.didProcessExit) {
            throw new driver_1.errors.InvalidContextError(`'${method} ${url}' cannot be proxied to WinAppDriver server because ` +
...
class WADProcess {
    ...
    async start() {
        ...
        if (!this.port) {
            await PORT_ALLOCATION_GUARD(async () => {
                ...
                try {
                    ...
                    throw this.log.errorWithException(`Could not find any free port in range ${startPort}..${endPort}. ` +
        ...
        this.proc = new teen_process_1.SubProcess(this.executablePath, args, {
            encoding: 'ucs2'
        });
        this.proc.on('output', (stdout, stderr) => {
            const line = lodash_1.default.trim(stderr || stdout);
            if (line) {
                this.log.debug(line);
            }
        });
...
class WinAppDriver {
    ...
    async _prepareSessionWithBuiltInServer(isForceQuitEnabled) {
        const executablePath = await (0, installer_1.getWADExecutablePath)();
        ...
        try {
            ...
            let errorMessage = (`WinAppDriver server '${executablePath}' is not listening at ${serverUrl} ` +
                `after ${STARTUP_TIMEOUT_MS}ms timeout. Make sure it could be started manually.`);
            if (await this.proxy.isListening()) {
                errorMessage = (`WinAppDriver server '${executablePath}' is listening at ${serverUrl}, ` +
    ...
    async _prepareSessionWithCustomServer(url) {
        ...
        try {
            ...
            throw new Error(`Cannot parse the provided WinAppDriver URL '${url}'. Original error: ${e.message}`);
        }
        /** @type {import('@appium/types').ProxyOptions} */
        const proxyOpts = {
            log: this.log,
            base: lodash_1.default.trimEnd(parsedUrl.pathname, '/'),
            server: parsedUrl.hostname,
            port: parseInt(parsedUrl.port, 10),
            scheme: lodash_1.default.trimEnd(parsedUrl.protocol, ':'),
        };
        if (this.opts.reqBasePath) {
            proxyOpts.reqBasePath = this.opts.reqBasePath;
        }
        ...
        try {
            ...
            let errorMessage = (`WinAppDriver server is not listening at ${url}. ` +
                `Make sure it is running and the provided wadUrl is correct`);
            if (await this.proxy.isListening()) {
                errorMessage = (`WinAppDriver server is listening at ${url}, but fails to respond with a proper status. ` +
    ...
    async _startSession(caps) {
        ...
        try {
            await (0, asyncbox_1.waitForCondition)(condFn, {
 ...
 */
//# sourceMappingURL=winappdriver.js.map